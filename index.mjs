...
const app = createApp({
    setup() {
        ...
        const openImage = async (image) => {
            ...
            //select layer
            layers.add("imagelayer", {
                image: new Image(canvas.value.width, canvas.value.height),
                clearable: true,
								internal: true,
                canvasWidth: canvas.value.width,
                canvasHeight: canvas.value.height,
        ...
        const activate = (type) => {
            switch (type) {
                ...
                case "box":
                    Pubsub.publish("eventId", "boxselect");
                    layers.getLastLayer().addSelect("boxselect", {
                        canvasWidth: canvas.value.width,
                        canvasHeight: canvas.value.height,
                    });
                    break;
